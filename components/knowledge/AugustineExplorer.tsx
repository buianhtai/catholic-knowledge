...
export default function AugustineExplorer(){
 ...
 return <main className={styles.page}>
  <section className={`container ${styles.hero}`}>
   <div className={styles.portraitPanel}><EditorialArtwork assetId="art.augustine-philippe-de-champaigne" height="100%" radius={22} objectPosition="center 20%"/><div className={styles.portraitMeta}><span>Saint · Bishop · Theologian</span><span>354–430</span></div></div>
  ...
  <section className={`container ${styles.body}`}>
   ...
   {tab==='Writings'&&<article className={styles.fullPanel}><div className="eyebrow">Featured writings</div><h2>Works in the graph</h2><div className={styles.books}>{writings.map(({entity})=>entity&&<div key={entity.id}><EditorialArtwork assetId="art.gutenberg-bible" height={150} radius={10} showCredit={false}/><h3>{text(entity.labels)}</h3><p>{entity.subtype}</p><b>Open work →</b></div>)}</div></article>}
   {tab==='Sources'&&<article className={styles.fullPanel}><div className="eyebrow">Provenance</div><h2>Evidence behind the profile</h2><p>Canonical entities and relationships reference known sources; generated explanations remain separate from evidence. Editorial artwork is sourced separately and never treated as factual evidence.</p><div style={{display:'flex',gap:10,flexWrap:'wrap',margin:'18px 0 24px'}}>{sources.map(source=><SourceBadge key={source.id} source={source}/>)}</div>{sources.map(source=><div className={styles.source} key={source.id}><a href={source.url} target="_blank" rel="noreferrer">{source.title} ↗</a><span>{source.publisher} · {source.sourceType} {source.license?`· ${source.license}`:''}</span></div>)}</article>}
  </section>
 </main>
}

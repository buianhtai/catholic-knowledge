...
export default function AugustineExplorer(){
 ...
 return <main className={styles.page}>
  <section className={`container ${styles.hero}`}>
   <div className={styles.portraitPanel}><img src="/demo/augustine.svg" alt="Editorial illustration of Saint Augustine" style={{width:'100%',height:'100%',minHeight:360,objectFit:'cover',borderRadius:22}}/><div className={styles.portraitMeta}><span>Saint · Bishop · Theologian</span><span>354–430</span></div></div>
  ...
  <section className={`container ${styles.body}`}>
   ...
   {tab==='Writings'&&<article className={styles.fullPanel}><div className="eyebrow">Featured writings</div><h2>Works in the graph</h2><div className={styles.books}>{writings.map(({entity})=>entity&&<div key={entity.id}><img src="/demo/scripture.svg" alt="" style={{width:'100%',height:100,objectFit:'cover',borderRadius:10,marginBottom:10}}/><h3>{text(entity.labels)}</h3><p>{entity.subtype}</p><b>Open work →</b></div>)}</div></article>}

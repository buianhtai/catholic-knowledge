      ...
      <div className={styles.workspace}>
        <section className={styles.hero}>
          ...
          <div className={styles.heroArt}>
            <img className={styles.heroIllustration} src="/demo/augustine.svg" alt="Editorial illustration of Saint Augustine with a halo and book"/>
          ...
          <aside className={styles.today}>
            <div className={styles.todayTitle}><img className={styles.todayThumb} src="/demo/liturgy.svg" alt=""/><div><strong>Today in the Church</strong><small>29 August</small></div></div>
        ...
        <section className={styles.categorySection}><div className={styles.sectionTitle}><h2>Explore the Catholic World</h2><Link href="/explore">Open knowledge map →</Link></div><div className={styles.categoryGrid}>{categories.map(({image,title,body,href})=><Link href={href} className={styles.category} key={title}><img className={styles.categoryThumb} src={image} alt=""/><div><strong>{title}</strong><small>{body}</small><em>Explore →</em></div></Link>)}</div></section>
        ...
        <section className={styles.dashboard}>
          <article className={styles.featureCard}><img className={styles.featureThumb} src="/demo/augustine.svg" alt=""/><span className="eyebrow">Featured · St. Augustine of Hippo</span><h2>{augustine?text(augustine.labels):'St. Augustine of Hippo'}</h2><p>Bishop, Doctor of the Church, and one of the most influential thinkers in Christian tradition.</p><div className={styles.stats}><span><b>354–430</b>Years</span><span><b>2</b>Featured writings</span><span><b>{relationCount}</b>Connections</span></div><Link href="/saints/augustine-of-hippo">Explore His Life →</Link></article>
          <Link href="/explore" className={styles.mapCard}><div className={styles.cardHead}><h2>Visual Knowledge Map</h2><span>Interactive</span></div><div className={styles.miniGraph}><i>Grace</i><i>God</i><strong><img className={styles.miniGraphPortrait} src="/demo/augustine.svg" alt="Augustine"/></strong><i>Monica</i><i>Confessions</i><i>Church</i></div><b>Explore Full Graph →</b></Link>
          ...
          <Link href="/learn/jesus-to-nicaea" className={styles.learnCard}><span className="eyebrow">Learning Journey</span><h2>Jesus to Nicaea</h2><img className={styles.journeyThumb} src="/demo/nicaea.svg" alt="Council and early Church illustration"/><p>Follow the essential journey of our faith in 6 steps.</p><div className={styles.progress}><i/></div><b>Continue Journey →</b></Link>
        </section>

        <section className={styles.bottomGrid}><div className={styles.topics}><h2>Topics You Might Explore</h2><div>{topics.map(({image,label,href})=><Link href={href} key={label}><img className={styles.topicThumb} src={image} alt=""/><span>{label}</span></Link>)}</div></div><aside className={styles.library}><h2>From the Library</h2><div><img className={styles.libraryThumb} src="/demo/scripture.svg" alt="Open Scripture illustration"/><p><b>Confessions</b><small>St. Augustine</small><em>The restless heart and the mercy of God.</em></p></div></aside></section>
      </div>
    </section>
  </main>;
}

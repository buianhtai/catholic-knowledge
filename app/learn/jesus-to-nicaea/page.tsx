...
const chapters = [
  {year:'k. 30',kicker:'Khởi nguyên',title:'Chúa Giêsu và các Tông đồ',body:'Từ Galilê đến Giêrusalem, lời rao giảng về Nước Thiên Chúa quy tụ một cộng đoàn môn đệ. Từ đây, Tin Mừng bắt đầu lan vượt ra ngoài biên giới của một vùng đất.',asset:'art.gutenberg-bible',nodes:['Galilê','Nhóm Mười Hai','Giêrusalem','Tin Mừng'],tone:196},
  ...
  {year:'30–60',kicker:'Những con đường',title:'Phêrô, Phaolô và sứ mạng',body:'Từ Giêrusalem, Antiôkia đến Rôma, các tuyến đường quanh Địa Trung Hải trở thành mạng lưới truyền giáo. Những cộng đoàn mới cũng đặt ra những câu hỏi mới về đức tin và đời sống Kitô hữu.',asset:'art.gutenberg-bible',nodes:['Phêrô','Phaolô','Antiôkia','Rôma'],tone:174},
  {year:'64–303',kicker:'Chứng tá',title:'Bách hại và tăng trưởng',body:'Đức tin không lớn lên trong an toàn. Chứng tá của các vị tử đạo, các nhà hộ giáo và các Giáo Hội địa phương góp phần làm cho căn tính Kitô giáo ngày càng rõ nét.',asset:'art.nicaea-icon',nodes:['Các vị tử đạo','Hộ giáo','Rôma','Giáo Hội địa phương'],tone:130},
  ...
  {year:'325',kicker:'Hội tụ',title:'Công đồng Nixêa',body:'Tại Nixêa, các giám mục đối diện với một câu hỏi mang tính quyết định về căn tính của Đức Kitô. Kinh Tin Kính trở thành điểm quy tụ giúp các cộng đoàn cùng tuyên xưng một đức tin.',asset:'art.nicaea-icon',nodes:['Nixêa','Ariô','Athanasiô','Kinh Tin Kính'],tone:246},
] as const;
...
export default function JesusToNicaeaPage(){
 ...
 return <main className={styles.page}>
   ...
   <section className={styles.prologue}>
     <div className={styles.prologueCopy}><span>Hành trình xuyên qua lịch sử đức tin</span><h1>Từ Chúa Giêsu<br/><em>đến Nixêa</em></h1><p>Đừng đọc lịch sử như một chuỗi ngày tháng rời rạc. Hãy bước vào mạng lưới sống động của con người, địa danh, biến cố và tư tưởng đã định hình Giáo Hội sơ khai.</p><div className={styles.prologueActions}><button onClick={()=>jump(0)}>Bắt đầu hành trình ↓</button><button className={styles.soundIntro} onClick={toggleSound}>{soundOn?'Tắt không gian âm thanh':'Bật không gian âm thanh ♫'}</button></div></div>
   ...
   <div className={styles.journey}>
     ...
     <div className={styles.scenes}>
       {chapters.map((chapter,index)=><section ref={el=>{sceneRefs.current[index]=el}} data-index={index} key={chapter.title} className={`${styles.scene} ${index===active?styles.sceneActive:''}`}>
         ...
         <div className={styles.sceneCopy}><span className={styles.kicker}>{chapter.kicker} · {chapter.year}</span><div className={styles.sceneNumber}>{String(index+1).padStart(2,'0')}</div><h2>{chapter.title}</h2><p>{chapter.body}</p><div className={styles.constellation}>{chapter.nodes.map((node,nodeIndex)=><span style={{animationDelay:`${nodeIndex*100}ms`}} key={node}><i/> {node}</span>)}</div>{index===chapters.length-1&&<Link className={styles.enter} href="/cong-dong/nixea">Khám phá Công đồng Nixêa →</Link>}</div>

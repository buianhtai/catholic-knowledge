...
const chapters = [
  {id:'chapter-1',year:'k. 30',kicker:'Khởi nguyên',title:'Chúa Giêsu và các Tông đồ',body:'Từ Galilê đến Giêrusalem, lời rao giảng về Nước Thiên Chúa quy tụ một cộng đoàn môn đệ. Từ đây, Tin Mừng bắt đầu lan vượt ra ngoài biên giới của một vùng đất.',asset:'art.gutenberg-bible',nodes:['Galilê','Nhóm Mười Hai','Giêrusalem','Tin Mừng'],tone:196},
  {id:'chapter-2',year:'k. 30',kicker:'Ngọn lửa',title:'Lễ Ngũ Tuần',body:'Các môn đệ bước ra khỏi căn phòng đóng kín. Việc loan báo Tin Mừng trở thành một sứ mạng công khai, và cộng đoàn Giáo Hội bắt đầu mở rộng.',asset:'art.mass-at-bolsena',nodes:['Chúa Thánh Thần','Phêrô','Phép Rửa','Công vụ'],tone:220},
  {id:'chapter-3',year:'30–60',kicker:'Những con đường',title:'Phêrô, Phaolô và sứ mạng',body:'Từ Giêrusalem, Antiôkia đến Rôma, các tuyến đường quanh Địa Trung Hải trở thành mạng lưới truyền giáo. Những cộng đoàn mới cũng đặt ra những câu hỏi mới về đức tin và đời sống Kitô hữu.',asset:'art.gutenberg-bible',nodes:['Phêrô','Phaolô','Antiôkia','Rôma'],tone:174},
  {id:'chapter-4',year:'64–303',kicker:'Chứng tá',title:'Bách hại và tăng trưởng',body:'Đức tin không lớn lên trong an toàn. Chứng tá của các vị tử đạo, các nhà hộ giáo và các Giáo Hội địa phương góp phần làm cho căn tính Kitô giáo ngày càng rõ nét.',asset:'art.nicaea-icon',nodes:['Các vị tử đạo','Hộ giáo','Rôma','Giáo Hội địa phương'],tone:130},
  {id:'chapter-5',year:'313',kicker:'Bước ngoặt',title:'Constantinô và một thế giới đổi thay',body:'Khi các Kitô hữu có thể công khai sống đức tin hơn, Giáo Hội bước vào một giai đoạn lịch sử mới. Những tranh luận thần học giờ đây có thể quy tụ các giám mục từ nhiều miền khác nhau.',asset:'art.nicaea-icon',nodes:['Constantinô','Milan','Giám mục','Đế quốc'],tone:164},
...
export default function JesusToNicaeaPage(){
 ...
 useEffect(()=>{const engine=soundRef.current;if(!engine)return;const now=engine.ctx.currentTime;const base=chapters[active].tone;engine.osc.frequency.cancelScheduledValues(now);engine.harmonic.frequency.cancelScheduledValues(now);engine.osc.frequency.linearRampToValueAtTime(base,now+1.6);engine.harmonic.frequency.linearRampToValueAtTime(base*1.5,now+1.8)},[active]);
 ...
 return <main className={styles.page}>
   ...
   <div className={styles.journey}><aside className={styles.timeline} aria-label="Dòng thời gian hành trình"><span className={styles.timelineLabel}>30 → 325</span><div className={styles.timelineTrack}><i style={{height:`${(active/(chapters.length-1))*100}%`}}/></div>{chapters.map((chapter,index)=><button key={chapter.title} onClick={()=>jump(index)} className={index===active?styles.timelineActive:''}><span>{chapter.year}</span><small>{chapter.kicker}</small></button>)}</aside>
     <div className={styles.scenes}>{chapters.map((chapter,index)=><section id={chapter.id} ref={el=>{sceneRefs.current[index]=el}} data-index={index} key={chapter.title} className={`${styles.scene} ${index===active?styles.sceneActive:''}`}><div className={styles.sceneArt}><EditorialArtwork assetId={chapter.asset} height={900} radius={0} showCredit={false}/><div className={styles.sceneShade}/></div><div className={styles.sceneCopy}><span className={styles.kicker}>{chapter.kicker} · {chapter.year}</span><div className={styles.sceneNumber}>{String(index+1).padStart(2,'0')}</div><h2>{chapter.title}</h2><p>{chapter.body}</p><div className={styles.constellation}>{chapter.nodes.map((node,nodeIndex)=><span style={{animationDelay:`${nodeIndex*100}ms`}} key={node}><i/> {node}</span>)}</div><div className={styles.storyHandoff}><Link href={toExploreHref({journeyId:'jesus-to-nicaea',chapterId:chapter.id,chapterLabel:chapter.title})}>Khám phá thời điểm này ↗</Link>{index===chapters.length-1&&<Link className={styles.enter} href="/cong-dong/nixea">Đi sâu vào Công đồng Nixêa →</Link>}</div></div><div className={styles.depth} aria-hidden="true">{chapter.nodes.map((node,nodeIndex)=><span key={node} style={{'--x':`${18+(nodeIndex*21)%68}%`,'--y':`${16+(nodeIndex*27)%72}%`,'--d':`${.65+nodeIndex*.12}`} as React.CSSProperties}>{node}</span>)}</div></section>)}</div>
   </div>
   <section className={styles.epilogue}><div className={styles.pullback}><span>325</span><i/><i/><i/><i/><i/><i/><i/></div><div><span>Đây mới chỉ là một chòm sao.</span><h2>Cả đức tin là<br/>một vũ trụ kết nối.</h2><p>Mỗi con người dẫn đến một địa danh. Mỗi địa danh mở ra một biến cố. Mỗi biến cố lại kết nối với giáo lý, tác phẩm và những hành trình khác.</p><Link href="/kham-pha">Mở bản đồ tri thức →</Link></div></section>
 </main>
}
